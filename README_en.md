...
6. **Secrets never leave the machine** — dumps are scanned before archiving (`CONTAINS SECRETS`), and the staged diff is scanned again before every push; a credential that did get pushed is treated as burned — rotate first.
7. **Restore is one copy command** — the manifest records the original path. No tooling, no lock-in.
